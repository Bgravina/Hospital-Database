...
void save_patient (patient_t patient_lib[], int count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        fwrite(patient_lib[i].surname, char_len, sizeof(patient_lib[i].surname), fp);
        fwrite(patient_lib[i].firstname, char_len, sizeof(patient_lib[i].firstname), fp);
        ...
        fwrite(patient_lib[i].location_dt.department, char_len, sizeof(patient_lib[i].location_dt.department), fp);
        ...
        fwrite(patient_lib[i].diagnosis.med, char_len, sizeof(patient_lib[i].diagnosis.med), fp);
...
int load_patient (patient_t patient_lib[], int count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        fread(patient_lib[i].surname, char_len, sizeof(patient_lib[i].surname), fp);
        fread(patient_lib[i].firstname, char_len, sizeof(patient_lib[i].firstname), fp);
        ...
        fread(patient_lib[i].location_dt.department, char_len, sizeof(patient_lib[i].location_dt.department), fp);
        ...
        fread(patient_lib[i].diagnosis.med, char_len, sizeof(patient_lib[i].diagnosis.med), fp);
...
int valid_fullname(patient_t patient_lib[],int count, debug_t debug)
{
    ...
    for (i=1; i < lenF; i++)
  	{
  		/* Condition for checking if any of the characters are notdebug.add_patient_count,
           debug.display_patient_count,
           debug.update_patient_count,
           debug.save_patient_count,
           debug.load_patient_count
...
/* comparing each string with the one next to it */
    for (i = 1; i < count; i++)
    {
        for (j = 1; j < count; j++)
        {
            ...
            compfirst = strcmp(patient_lib[j-1].firstname, patient_lib[j].firstname);
            /* when the result is positive integrer */
            if ( (comp > 0) || (compfirst >0))
            {
                ...
                swap_int(&patient_lib[j-1].arrival_dt.day, &patient_lib[j].arrival_dt.day);
                swap_int(&patient_lib[j-1].arrival_dt.month, &patient_lib[j].arrival_dt.month);
                swap_int(&patient_lib[j-1].arrival_dt.year, &patient_lib[j].arrival_dt.year);
                swap_int(&patient_lib[j-1].arrival_dt.hour, &patient_lib[j].arrival_dt.hour);
                swap_int(&patient_lib[j-1].arrival_dt.minute, &patient_lib[j].arrival_dt.minute);
                ...
                swap_int(&patient_lib[j-1].departure_dt.hour, &patient_lib[j].departure_dt.hour);
                swap_int(&patient_lib[j-1].departure_dt.minute, &patient_lib[j].departure_dt.minute);
